...
fn main() {
    ...
    for k in 5..=21 {
        println!("K = {}", k);
        let entries = parse_csv_to_entries::<&str, N_ASSETS, 8>(&format!(
            "src/csv/two_assets/two_assets_entry_2_{}.csv",
            k
        ))
        .unwrap();
        ...
        for chunk in entries.chunks(chunk_size).map(|c| c.to_vec()) {
            ...
            let handle = thread::spawn(move || {
                ...
                for entry in chunk {
                    ...
                    local_result.push(
                        Fp::from_raw_bytes(entry.username().as_bytes()).unwrap_or(Fp::zero()),
                    );
                    for balance in entry.balances().iter() {
                        local_result
                            .push(Fp::from_str_vartime(&balance.to_str_radix(10)[..]).unwrap());
                    }
                }
                local_result
            });
