    ...
    [usize; N_ASSETS + 1]: Sized,
{
     ...
     */
    pub fn new(entry: Vec<Entry<N_ASSETS>>, pp: pp, k: u32) -> Self {
        let w = root_of_unity(k);
        let mut idxs = vec![Fr::one()];
        for _ in 1..2u32.pow(k) as usize {
            idxs.push(idxs.last().unwrap() * w);
        }
        // TODO: Entry to polynomials
        let mut data: Vec<Fr> = vec![Fr::zero()];

        // iterating entry and pull out data from
        for e in entry.iter() {
            let node = e.compute_leaf();
            data.push(Fr::from_bytes_le(&node.hash.to_bytes()));
            data.push(Fr::from_bytes_le(&e.username_to_big_uint().to_bytes_le()));
            node.balances
                .iter()
                .for_each(|b| data.push(Fr::from_bytes_le(&b.to_bytes())));
        }
